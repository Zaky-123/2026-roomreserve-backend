...
public class BookingsController : ControllerBase
{
    ...
    public async Task<ActionResult<BookingResponseDto>> CreateBooking(CreateBookingDto createDto)
    {
        try
        {
            ...
            // Catat history pembuatan booking
    ...
    public async Task<ActionResult<StatusUpdateResponseDto>> UpdateBookingStatus(int id, [FromBody] UpdateStatusDto statusDto)
    {
        try
        {
            ...
            // Parse status dari string ke enum
            if (!Enum.TryParse<BookingStatus>(statusDto.NewStatus, true, out var newStatus))
            {
                return BadRequest($"Status '{statusDto.NewStatus}' tidak valid. Pilihan: Pending, Approved, Rejected, Cancelled, Completed");
            }
            ...
            {
                return BadRequest($"Tidak dapat mengubah status dari {oldStatus} ke {newStatus}");
            }
            ...
            var response = new StatusUpdateResponseDto
            {
    ...
    public async Task<ActionResult<IEnumerable<BookingHistoryDto>>> GetBookingHistory(int id)
    {
        ...
        {
            _logger.LogError(ex, "Error getting booking history {Id}", id);
            return StatusCode(500, "Internal server error");
        }
    }

    // Validasi transisi status
    private bool IsValidStatusTransition(BookingStatus current, BookingStatus newStatus)
    {
        // Tidak boleh sama
        ...
        {
            // Pending bisa ke Approved, Rejected, atau Cancelled
            ...
            // Approved bisa ke Completed atau Cancelled
            (BookingStatus.Approved, BookingStatus.Completed) => true,
            (BookingStatus.Approved, BookingStatus.Cancelled) => true,
            
            // Rejected tidak bisa ke status lain (final)
            (BookingStatus.Rejected, _) => false,
            
            // Cancelled tidak bisa ke status lain (final)
            (BookingStatus.Cancelled, _) => false,
            
            // Completed tidak bisa ke status lain (final)
            (BookingStatus.Completed, _) => false,
            
            // Default: semua transisi lain tidak valid
            _ => false
        };
    }
}